...
#         client = razorpay.Client(auth=("rzp_test_sqVr1JaTu9Hdjg","j1UEdkT5iAYQMED7CQr9qQ1s"))
#         data = {"amount":razoramount,"currency":"INR","receipt":"order_recptid_11"}
#         payment_response = client.order.create(data=data)
#         print(payment_response)
#         #{'id': 'order_OBBvlKCrur3Hfx', 'entity': 'order', 'amount': 37500, 'amount_paid': 0, 'amount_due': 37500, 'currency': 'INR', 'receipt': 'order_recptid_11', 'offer_id': None, 'status': 'created', 'attempts': 0, 'notes': [], 'created_at': 1715868237}
...
#     user = request.user
#     #returns redirect ('orders')
#     customer = Customer.objects.get(id = cust_id)
#     #To update payment status and payment_id
#     payment = Payment.objects.get(rezorpay_order_id = order_id)
#     payment.paid = True
#     payment_razorpay_payment_id = payment_id
#     payment.save()
...
#     cart = Cart.objects.filter(user=user)
#     for c in cart:
#         o = OrderPlaced.objects.create(user=user, customer=customer, product=c.product, quantity = c.quantity, payment=payment)
#         o.save()
#         c.delete()
